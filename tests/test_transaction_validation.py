...
class TestTransactionValidation(unittest.TestCase):
    def test_special_format_parsing(self):
        ...
        result = validate_transaction(
            "TX-2023/SP-001|USER:john.doe|AMT:$1,234.56|TYPE:SPECIAL|DESC:Test & Validation"
        )
...
DESC:Multiple\nLine\nTransaction"""
        ...
        result = validate_transaction(
            'TX-2023/SP-003|{"user":"bob.jones","amount":"¥5000","type":"JSON_LIKE","desc":"JSON format test"}'
        )
        self.assertTrue(result.is_valid)
    ...
    def test_complex_international(self):
        ...
        result = validate_transaction(
            "TX-2023/INT-001|USER:hans.mueller|AMT:€1.000.000,00|TYPE:LARGE_EU|DESC:European large amount"
        )
        self.assertTrue(result.is_valid)
        ...
        # Test Arabic format
        result = validate_transaction(
            "TX-2023/INT-002|USER:ahmed.hassan|AMT:١٢٣٤٥.٦٧|TYPE:ARABIC|DESC:Arabic format transaction تحويل"
        )
        self.assertTrue(result.is_valid)
        ...
        # Test Indian format
        result = validate_transaction(
            "TX-2023/INT-004|USER:raj.patel|AMT:₹1,23,45,678.90|TYPE:INR|DESC:Indian format with lakhs"
        )
        self.assertTrue(result.is_valid)
    ...
    def test_encoding_variants(self):
        ...
        # Test UTF-8 BOM
        result = validate_transaction(
            "\uFEFFTX-2023/ENC-001|USER:maria.garcía|AMT:$100.00|TYPE:UTF8_BOM|DESC:BOM test case"
        )
        self.assertTrue(result.is_valid)
        ...
        result = validate_transaction(
            "TX-2023/ENC-003|USER:james.smith|AMT:$200.00|TYPE:EMOJI|DESC:Payment for 🎉 party 🎈"
        )
        self.assertTrue(result.is_valid)
        ...
        # Test mixed encoding
        result = validate_transaction(
            "TX-2023/ENC-004|USER:софія.коваль|AMT:₴300.00|TYPE:MIXED_ENC|DESC:Український опис 💰"
        )
        self.assertTrue(result.is_valid)
    ...
    def test_invalid_format_handling(self):
        ...
        result = validate_transaction(
            "TX-2023/INV-001|USER:john.doe|TYPE:INVALID|DESC:Missing amount field"
        )
        ...
        result = validate_transaction(
            "TX-2023/INV-002|USER:jane.smith|AMT:1234.5.6|TYPE:INVALID|DESC:Invalid number format"
        )
        self.assertFalse(result.is_valid)
        ...
        result = validate_transaction(
            "TX-2023/INV-003||USER:bob||AMT:$100.00|TYPE:INVALID||DESC:Extra delimiters"
        )
        self.assertFalse(result.is_valid)
