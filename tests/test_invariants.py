...
class InvariantResults:
    ...
    def calculate_scores(self) -> Dict[str, float]:
        ...
        for pdf_name, invariants in self.results.items():
            ...
            self.scores[pdf_name] = (passed_tests / total_tests * 100) if total_tests > 0 else 0
        return self.scores
...
def test_invariant_row_count_sanity(request):
    ...
    for pdf_name in candidate_pdfs:
        ...
        try:
            ...
            if not passed:
                print(f"❌ {pdf_name}: Row count {row_count} outside valid range [1, 250]")
            else:
...
def test_invariant_valid_categories(request):
    ...
    valid_categories = {
        "PAGAMENTO", "AJUSTE", "ENCARGOS", "SERVIÇOS", "SUPERMERCADO",
        "FARMÁCIA", "RESTAURANTE", "POSTO", "TRANSPORTE", "TURISMO",
        "ALIMENTAÇÃO", "SAÚDE", "VEÍCULOS", "VESTUÁRIO", "EDUCAÇÃO",
        "HOBBY", "FX", "DIVERSOS", "INTERNACIONAL", "ENCARGO"
    }
...
def test_invariant_date_format(request):
    ...
    for pdf_name in candidate_pdfs:
        ...
        try:
            ...
            if not passed:
                print(f"❌ {pdf_name}: Invalid dates: {invalid_dates[:5]}")  # Show first 5
            else:
