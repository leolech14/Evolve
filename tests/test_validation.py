...
def validate_categories(rows: list[dict]) -> list[str]:
    ...
    VALID_CATEGORIES = {
        'AJUSTE', 'DIVERSOS', 'FARMÁCIA', 'FX', 'PAGAMENTO',
        'ALIMENTAÇÃO', 'SAÚDE', 'VEÍCULOS', 'VESTUÁRIO', 'EDUCAÇÃO',
        'SERVIÇOS', 'SUPERMERCADO', 'RESTAURANTE', 'POSTO', 'TRANSPORTE',
        'TURISMO', 'ENCARGOS', 'HOBBY'
    }
    invalid = []
    for row in rows:
...
def test_all_statements():
    ...
    for pdf_file in test_files:
        ...
        accuracy = (min(csv_total, pdf_total) / max(csv_total, pdf_total) * 100) \
            if pdf_total > 0 else Decimal('0')
