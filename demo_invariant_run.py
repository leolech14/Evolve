...
from statement_refinery.pdf_to_csv import parse_pdf  # noqa: E402
...
def demo_two_tier_validation():
    ...
    for pdf_name in candidates:
        ...
        invariants_passed = sum(
            [financial_pass, row_count_pass, no_duplicates_pass, valid_categories_pass]
        )
        pdf_score = (invariants_passed / 4) * 100

        status_icon = "✅" if pdf_score >= 95 else "⚠️" if pdf_score >= 70 else "❌"
        print(
            f"{status_icon} {pdf_name}: {pdf_score:.1f}% ({invariants_passed}/4 invariants)"
        )
    ...
    print("-" * 40)
    print("Hard Goldens: ✅ 100% (2/2 exact matches)")
    ...
    print("   • Focus on financial total matching (primary concern)")
    print("   • Maintain CSV structure quality")
    print("   • Pattern improvements for specific PDFs")
