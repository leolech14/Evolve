...
class IncrementalLearner:
    ...
    def analyze_missing_transactions(self, pdf_path: Path, validation_results: Dict) -> List[Dict]:
    ...
    def _generate_pattern_from_examples(self, structure: str, examples: List[str]) -> Optional[Dict]:
    ...
    def _classify_pattern_type(self, structure: str, examples: List[str]) -> Optional[Dict]:
        ...
        # Fee or charge line
        if "AMOUNT" in structure and any(word in first_example for word in ["TAXA", "TARIFA", "JUROS", "MULTA"]):
            return {
                "name": "fee_charge",
                "handler": "parse_fee",
        ...
        if "AMOUNT" in structure and any(word in first_example for word in ["USD", "EUR", "DOLAR"]):
            return {
    ...
    def _build_regex_pattern(self, pattern_info: Dict, examples: List[str]) -> Optional[str]:
    ...
    def generate_enhanced_patterns(self, pdf_paths: List[Path]) -> List[Dict]:
        ...
        for pdf_path in pdf_paths:
            ...
            spec = importlib.util.spec_from_file_location("semantic_validator", 
                                                         ROOT / "scripts" / "semantic_validator.py")
            ...
            success_rate = file_info["parsed_transactions"] / max(1, file_info["total_lines"])
            ...
            if structure not in seen_structures:
                ...
                for existing in unique_patterns:
                    if existing["structure"] == structure:
                        ...
                        existing["examples"] = existing["examples"][:5]  # Keep only top 5
...
def main():
    parser = argparse.ArgumentParser(description="Incremental learning for parser improvement")
    ...
    results = {
        ...
        "high_confidence_patterns": len([p for p in patterns if p["confidence"] >= 0.6]),
        "patterns": patterns,
    ...
    print(f"High-confidence patterns: {len([p for p in patterns if p['confidence'] >= 0.6])}")
    
    if patterns:
        print("\nTOP 5 LEARNED PATTERNS:")
        for i, pattern in enumerate(patterns[:5], 1):
            print(f"{i}. {pattern['name']} (confidence: {pattern['confidence']:.1%})")
            print(f"   Example: {pattern['examples'][0] if pattern['examples'] else 'N/A'}")
