    ...
    try:
        ...
        try:
            ...
            if accuracy < 0.95:
                ...
                result["diagnostic_summary"] = f"Missing {delta:.2f} BRL ({(1-accuracy)*100:.1f}% of total)"
            else:
...
def main():
    ...
    if training_targets:
        ...
        for target in sorted(training_targets, 
                           key=lambda x: x["financial_accuracy"]["accuracy_percentage"] if x["financial_accuracy"] else 0):
            if target["financial_accuracy"]:
                acc = target["financial_accuracy"]["accuracy_percentage"]
                delta = target["financial_accuracy"]["delta"]
                print(f"   • {target['pdf_name']}: {acc:.1f}% (missing {delta:.2f} BRL)")
