...
class PatternEnhancer:
    ...
    def _is_embedded_transaction_pattern(
        self, structure: str, examples: List[str]
    ) -> bool:
    ...
    def generate_pattern_report(self) -> Dict:
        ...
        return {
            ...
            "implementation_priority": sorted(
                solutions, key=lambda x: x["count"], reverse=True
            )[:5],
            "next_steps": [
...
def main():
    ...
    if not analysis_file.exists():
        print(
            f"Analysis file {analysis_file} not found. Run comprehensive_analysis.py first."
        )
        return 1
    ...
    print("=" * 60)
    print(
        f"New patterns generated: {report['enhancement_summary']['new_patterns_generated']}"
    )
    print(
        f"Lines potentially covered: {report['enhancement_summary']['lines_potentially_covered']}"
    )
    print(
        f"Potential success improvement: {report['enhancement_summary']['potential_success_improvement']}"
    )
