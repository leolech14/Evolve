...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Batch parse PDFs to CSV files")
    parser.add_argument(
        "--out", type=Path, required=True, help="Output directory for CSV files"
    )
    parser.add_argument(
        "--candidates-only",
        action="store_true",
    ...
    parser.add_argument(
        "--goldens-only", action="store_true", help="Parse only hard golden PDFs"
    )
