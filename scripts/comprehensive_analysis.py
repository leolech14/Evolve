...
class PatternAnalyzer:
    ...
    def analyze_pdf(self, pdf_path: Path) -> Dict:
        ...
        try:
            ...
            # Calculate CSV total
            csv_total = sum(t.get("amount_brl", Decimal("0")) for t in parsed_transactions)
            ...
            analysis = {
                ...
                "success_rate": len(parsed_transactions) / max(1, len(parsed_transactions) + len(failed_lines))
            }
    ...
    def generate_report(self) -> Dict:
        ...
        return {
            "summary": {
                ...
                "overall_success_rate": self.parsing_stats["total_parsed"] / max(1, self.parsing_stats["total_parsed"] + self.parsing_stats["total_failed"])
            },
            "pattern_insights": {
            ...
            "totals_extraction": {
                ...
                "failed_extractions": self.parsing_stats["total_pdfs"] - len(self.totals_found),
...
def main():
    parser = argparse.ArgumentParser(description="Comprehensive PDF analysis for pattern discovery")
    parser.add_argument("pdf_dir", help="Directory containing PDF files")
    parser.add_argument("--output", "-o", default="diagnostics/comprehensive_analysis.json", 
    ...
    for pdf_path in sorted(pdf_dir.glob("*.pdf")):
        ...
        if "error" not in analysis:
            ...
            logger.info(f"{pdf_path.name}: {analysis['parsed_transactions']} parsed, "
    ...
    print(f"Unique Amount Formats: {report['pattern_insights']['unique_amount_formats']}")
    print(f"Discovered Patterns: {len(report['discovered_patterns'])}")
    print(f"Total Extractions Successful: {report['totals_extraction']['successful_extractions']}/{report['summary']['total_pdfs_analyzed']}")
    
    if report['discovered_patterns']:
        print("\nMOST COMMON FAILED PATTERNS:")
        for i, pattern in enumerate(sorted(report['discovered_patterns'], key=lambda x: x['count'], reverse=True)[:5]):
            print(f"{i+1}. Structure: {pattern['structure']} (appears {pattern['count']} times)")
