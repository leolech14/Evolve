...
class SemanticValidator:
    ...
    def validate_pdf_parsing(self, pdf_path: Path) -> Dict:
        ...
        validation_results = {
            ...
            "missing_transactions": self._detect_missing_transactions(all_lines, parsed_transactions),
            "total_reconciliation": self._validate_total_reconciliation(parsed_total, pdf_total),
    ...
    def _validate_business_logic(self, transactions: List[Dict]) -> Dict:
        ...
        for i, txn in enumerate(transactions):
            ...
            if category == "PAGAMENTO" and amount >= 0:
                violations.append(f"Transaction {i}: Payment should be negative, got {amount}")
                stats["positive_payment"] += 1
            
            # Charges should be positive
            if category == "ENCARGO" and amount <= 0:
                violations.append(f"Transaction {i}: Charge should be positive, got {amount}")
            ...
            if category == "INTERNACIONAL":
                if not txn.get("currency_orig"):
                    violations.append(f"Transaction {i}: International transaction missing currency")
                    stats["missing_currency"] += 1
                if txn.get("amount_orig", Decimal("0")) == 0:
                    violations.append(f"Transaction {i}: International transaction missing original amount")
            ...
            if inst_seq > 0 and (inst_tot == 0 or inst_seq > inst_tot):
                violations.append(f"Transaction {i}: Invalid installment {inst_seq}/{inst_tot}")
        ...
        return {
            ...
            "business_logic_score": max(0, 1 - len(violations) / max(1, len(transactions)))
        }
    
    def _detect_missing_transactions(self, all_lines: List[str], parsed_transactions: List[Dict]) -> Dict:
        ...
        for line_num, line in enumerate(all_lines, 1):
            ...
            if line_hash not in parsed_hashes:
                ...
                has_amount = bool(pdf_to_csv.re.search(r'\d{1,3}(?:\.\d{3})*,\d{2}', line))
                ...
                if (has_amount or has_date) and not has_skip_keyword:
                    potentially_missing.append({
                        ...
                        "has_date": has_date,
                        "confidence": "high" if (has_amount and has_date) else "medium"
                    })
        
        # Sort by confidence
        potentially_missing.sort(key=lambda x: (x["confidence"] == "high", x["has_amount"], x["has_date"]), reverse=True)
        
        return {
            "total_potentially_missing": len(potentially_missing),
            "high_confidence_missing": len([m for m in potentially_missing if m["confidence"] == "high"]),
            "missing_lines": potentially_missing[:30],  # Top 30 candidates
            "missing_rate": len(potentially_missing) / max(1, len(all_lines))
        }
    
    def _validate_total_reconciliation(self, parsed_total: Decimal, pdf_total: Optional[Decimal]) -> Dict:
    ...
    def _validate_categorization(self, transactions: List[Dict]) -> Dict:
        ...
        category_counts = Counter(txn.get("category", "UNKNOWN") for txn in transactions)
        ...
        if category_counts.get("UNKNOWN", 0) > 0:
            issues.append(f"{category_counts['UNKNOWN']} transactions with unknown category")
            ...
            if category_counts.get(category, 0) == 0:
                ...
                for txn in transactions:
                    ...
                    if any(pattern in desc for pattern in patterns):
                        issues.append(f"Found {category} transactions not properly categorized")
                        break
...
def main():
    ...
    if total_val["delta"]:
        ...
        print(f"  Delta: R$ {total_val['delta']:.2f} ({total_val['delta_percentage']:.1f}%)")
    ...
    missing = validation_results["missing_transactions"]
    print(f"Potentially missing transactions: {missing['total_potentially_missing']} "
