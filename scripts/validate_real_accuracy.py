...
def validate_real_accuracy(pdf_path: Path) -> dict:
    ...
    if total_available and pdf_total:
        ...
        total_accuracy = max(0, 1 - (total_delta / abs(pdf_total))) if pdf_total != 0 else 0
...
def main():
    parser = argparse.ArgumentParser(description="Real accuracy validation using PDF totals")
    parser.add_argument("pdf_dir", help="Directory containing PDF files")
    parser.add_argument("--output", default="diagnostics/real_accuracy.json", 
    ...
    for pdf_path in sorted(pdf_dir.glob("*.pdf")):
        ...
        print(f"\n📄 {result['pdf_name']}")
        print(f"   Coverage: {result['line_coverage']['coverage_rate']:.1%} ({quality['coverage_grade']})")
        ...
        if financial["total_available"]:
            ...
            print(f"   Financial: {financial['missing_percentage']:.1f}% missing ({quality['accuracy_grade']})")
            print(f"   Total: R$ {financial['parsed_total']:.2f} / R$ {financial['pdf_total']:.2f}")
        else:
            print(f"   Financial: Cannot validate (PDF total not extractable)")
        
        print(f"   Overall: {quality['overall_assessment']}")
    
    # Overall summary
    if validatable_pdfs > 0:
        overall_missing_pct = (total_missing_amount / total_pdf_amount * 100) if total_pdf_amount > 0 else 0
        overall_accuracy = max(0, 1 - (total_missing_amount / total_pdf_amount)) if total_pdf_amount > 0 else 0
    else:
    ...
    summary = {
        ...
        "overall_missing_percentage": float(overall_missing_pct) if overall_missing_pct else None,
    ...
    if overall_accuracy is not None:
        ...
        print(f"Missing amount: R$ {total_missing_amount:.2f} of R$ {total_pdf_amount:.2f}")
